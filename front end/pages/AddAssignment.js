...
import '../pageCss/AddAssignment.css'
...
const AddAssignment = () => {
    ...
    const addAssignmentToRecord = () => {
        if (name.length === 0) {
            ...
            const data ={"assignmentName":name,"endDate":enddate,"courseNo":courseid,"studentNo":studentid}

            axios.post(url + '/activity/assignment/addAssignment', data).then((response) => {
              const result = response.data
              if (result.status === 'success') {
                alert('successfully added new assignment to the record')
                history.push('/Activity')
              } else {
                console.log(result.error)
                alert('error while adding the assignment to the record')
              }
            })
          }
    }
    ...
    return <div>
        <h1>Schedule an Assignmnet</h1>
        <div class="mb-3">
        <label for="formGroupExampleInput" class="form-label">Name</label>
        <input onChange={(e)=>{setName(e.target.value)}}
        type="text" class="form-control" id="formGroupExampleInput" placeholder="Enter Assignment name">
        </input>
        </div>
        <div class="mb-3">
